...
//
//  the bottom needs four M3x?? countersunk screws for assembly
...
//   
// Jim Downloaded from: http://www.thingiverse.com/thing:779159/#files
//
...
pcbmntdia = 1*3.1;
pcbthrdia = 2.5+0.1; // for threads
pcbmntdx = pcb[0]-4.4*2-3.1;
pcbmntdy = pcb[1]-3.4*2-3.1;
...
buttondy = 38.0/3;
buttondia = 6.0;
buttonheight = 4.9+0.8; // above PCB, plus some extra tolerance

slider1pos = [13.5-pcb[0]/2, 18.0-pcb[1]/2]; // from lower left corner
sliderdy = (42.4-7.0)/2;
sliderdx = (9.6-5.4)/2 + 0.5; // add tolerance 
sliderdia = 6.0;
sliderheight = 1*4.5; // above PCB
slidersq = 2.0+0.2; // size of square slider button (with some extra tolerance)

trimmer1pos = [23.5-pcb[0]/2, 44.7-pcb[1]/2]; // from lower left corner
trimmerdy = 53.3-44.7;
trimmerdia = 3.1;
trimmerheight = 1*4.6; // above PCB
...
module top() {
	...
	difference() {
		union() {
			...
			translate([buttonrpos[0], buttonrpos[1], -(pcb2lcd-buttonheight-wall-tol)]) {
				cylinder(r = buttondia/2+tol+wall, h=(pcb2lcd-buttonheight-wall-tol)+d, $fn=60);
			}
			// slider frames
...
module button() {
	...
	translate([0, 0, wall/2]) cylinder(r = buttondia/2, h=(pcb2lcd-buttonheight)+wall/2, $fn=60);
	translate([0, 0, (pcb2lcd-buttonheight)+wall]) sphere(r = buttondia/2, $fn=60);
}
...
module button2() {
	...
	translate([0, 0, wall/2]) hull() {
		rotate([-ro, 0, 0]) cylinder(r = buttondia/2, h=(pcb2lcd-buttonheight)+wall/2, $fn=60);
		translate([0, buttondy, 0]) rotate([ro, 0, 0]) cylinder(r = buttondia/2, h=(pcb2lcd-buttonheight)+wall/2, $fn=60);
	}
	hull() {
		rotate([-ro, 0, 0]) translate([0, 0, (pcb2lcd-buttonheight)+wall]) sphere(r = buttondia/2, $fn=60);
		translate([0, buttondy/2, (pcb2lcd-buttonheight)+wall]) cylinder(r = buttondia/2, h=d, $fn=60);
	}
	hull() {
		translate([0, buttondy/2, (pcb2lcd-buttonheight)+wall]) cylinder(r = buttondia/2, h=d, $fn=60);
		translate([0, buttondy, 0]) rotate([ro, 0, 0]) translate([0, 0, (pcb2lcd-buttonheight)+wall]) sphere(r = buttondia/2, $fn=60);
	}
}
...
module slider() {
	difference() {
		union() {
			...
			cylinder(r = sliderdia/2, h=(pcb2lcd-sliderheight)+wall, $fn=60);
			translate([0, 0, (pcb2lcd-sliderheight)+wall]) sphere(r = buttondia/2, $fn=60);
		}
		translate([0, 0, -d]) {
