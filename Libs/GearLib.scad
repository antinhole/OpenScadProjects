...
gear (
	...
	thickness       = thickness/2,    //thickness of gear in mm
	hole_diameter   = hole_diameter,    //diameter of the hole in the center, in mm
	twist           = twist,    //teeth rotate this many degrees from bottom of gear to top.  360 makes the gear a screw with each thread going around once
	teeth_to_hide   = teeth_to_hide,    //number of teeth to delete to make this only a fraction of a circle
	pressure_angle  = pressure_angle,   //Controls how straight or bulged the tooth sides are. In degrees.
	clearance       = clearance,  //gap between top of a tooth on one gear and bottom of valley on a meshing gear (in millimeters)
	backlash        = backlash   //gap between two meshing teeth, in the direction along the circumference of the pitch circle
);
mirror([0, 0, 1]) 
...
translate([ 0,    0, 0]) 
    ...
    translate([ 0,    53, 0]) rotate([0,0, $t*360/n1])                 color([1.00,0.75,0.75]) 
    union(){
    gear(mm_per_tooth,n1,thickness,hole);
   translate([ 2.5,    0, 0]) { cube([2,5,thickness],center=true);}
   translate([ -2.5,    0, 0]) { cube([2,5,thickness],center=true);}
